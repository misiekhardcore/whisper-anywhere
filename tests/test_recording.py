...
class TestLiveVADLoop:
    MIN_AUDIO_BYTES: int = int(0.25 * 16000 * 2)

    @pytest.mark.asyncio
    async def test_speech_emits_partial(self) -> None:
        buffer: bytearray = bytearray(b"\x00\x01" * (8000 + 8000))
        ...
        output: TextOutput = TextOutput(False)
        recorder: Recorder = Recorder(model, None, _MockVAD(), output)
    ...
    @pytest.mark.asyncio
    async def test_buffer_too_short_no_calls(self) -> None:
        buffer: bytearray = bytearray(b"\x00\x01" * 1000)
        stop_event: asyncio.Event = asyncio.Event()
    ...
    @pytest.mark.asyncio
    async def test_no_speech_no_transcription(self) -> None:
        buffer: bytearray = bytearray(b"\x00\x01" * 10000)
        stop_event: asyncio.Event = asyncio.Event()
    ...
    @pytest.mark.asyncio
    async def test_silence_gap_emits_final(self) -> None:
        initial_audio: bytes = b"\x00\x01" * (8000 + 8000)
        buffer: bytearray = bytearray(initial_audio)
        stop_event: asyncio.Event = asyncio.Event()
        speech_done: bool = False

        SILENCE_THRESHOLD_BYTES: int = int(0.6 * 16000 * 2)
        ...
        grow_task: asyncio.Task = asyncio.create_task(_grow_buffer())
    ...
    @pytest.mark.asyncio
    async def test_recovers_from_vad_error(self) -> None:
        buffer: bytearray = bytearray(b"\x00\x01" * 20000)
        stop_event: asyncio.Event = asyncio.Event()
        call_count: int = 0
        ...
        model.transcribe.return_value = "hello"
        output: TextOutput = TextOutput(False)
        recorder: Recorder = Recorder(model, None, _FailingVAD(), output)
...
class TestFinishRecording:
    ...
    async def test_non_live_transcribes_full_buffer(self) -> None:
        ...
        output: TextOutput = TextOutput(False)
        recorder: Recorder = Recorder(model, None, None, output)
